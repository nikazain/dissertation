...
def evaluate_all_stages(model, tag):
    ...
    for stage in STAGES:
        ...
        np.savez(
            os.path.join(PROBS_DIR, f"cum_{tag}_stage{stage}.npz"),
...
def save(R, R_full, chosen_lrs, selection_log, branches, branches_full, note):
    out = {
        "note": note,
        "selection": "mean validation AUROC over stages 1..current",
    ...
    for stage in STAGES:
        ...
        for lr in LEARNING_RATES:
            ...
            selection_log[f"stage{stage}_lr{lr}"] = {
                "per_stage_val_auroc": per_stage_val,
                "mean": sel,
            }
            print(f"  lr {lr}: per-stage val {per_stage_val}  mean {sel:.4f}")
        ...
        print(f"\n  stage {stage} winner: lr {best_lr} (mean val auroc {best_sel:.4f})")
        model = best_model
